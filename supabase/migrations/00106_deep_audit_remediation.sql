...
-- rate_cards_unified is a view, not a table — skipping ALTER

ALTER TABLE recurring_invoices
  ADD COLUMN IF NOT EXISTS currency TEXT DEFAULT 'USD',
  ADD COLUMN IF NOT EXISTS created_by UUID REFERENCES auth.users(id);

-- service_rate_card_items is a view, not a table — skipping ALTER
...
-- workforce_rate_card_items is a view, not a table — skipping ALTER
...
DECLARE
  ...
  tbl_list TEXT[] := ARRAY[
    ...
    'project_budgets', 'api_keys', 'asset_documents',
...
DROP POLICY IF EXISTS "History tables are read-only for authenticated users" ON billing_invoice_items_history;
...
DROP POLICY IF EXISTS "History tables are read-only for authenticated users" ON budget_line_items_history;
CREATE POLICY "History tables are read-only for authenticated users"
  ON budget_line_items_history FOR SELECT TO authenticated
  USING (true);

DROP POLICY IF EXISTS "History tables are read-only for authenticated users" ON payment_attempts_history;
CREATE POLICY "History tables are read-only for authenticated users"
  ON payment_attempts_history FOR SELECT TO authenticated
  USING (true);

DROP POLICY IF EXISTS "History tables are read-only for authenticated users" ON settlements_history;
CREATE POLICY "History tables are read-only for authenticated users"
  ON settlements_history FOR SELECT TO authenticated
  USING (true);

DROP POLICY IF EXISTS "History tables are read-only for authenticated users" ON recurring_invoices_history;
CREATE POLICY "History tables are read-only for authenticated users"
