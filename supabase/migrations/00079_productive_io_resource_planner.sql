...
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE project_resources ADD COLUMN booking_type VARCHAR(30) DEFAULT 'confirmed'
            CHECK (booking_type IN ('confirmed', 'tentative', 'placeholder', 'soft_hold'));
        ALTER TABLE project_resources ADD COLUMN department VARCHAR(100);
        ALTER TABLE project_resources ADD COLUMN daily_rate DECIMAL(10,2);
        ALTER TABLE project_resources ADD COLUMN estimated_cost DECIMAL(14,2);
        ALTER TABLE project_resources ADD COLUMN budget_id UUID REFERENCES budgets(id) ON DELETE SET NULL;
        ALTER TABLE project_resources ADD COLUMN confirmed_at TIMESTAMPTZ;
        ALTER TABLE project_resources ADD COLUMN confirmed_by UUID REFERENCES users(id) ON DELETE SET NULL;
    END IF;
END $$;
