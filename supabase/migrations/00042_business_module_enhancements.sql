...
CREATE INDEX IF NOT EXISTS idx_pipelines_default ON pipelines(organization_id, is_default) WHERE is_default = TRUE;
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
        ...
        ALTER TABLE pipeline_stages ADD COLUMN IF NOT EXISTS rotting_days INTEGER DEFAULT 7;
    END IF;
END $$;
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
        ...
        ALTER TABLE activities ADD COLUMN IF NOT EXISTS occurred_at TIMESTAMPTZ DEFAULT NOW();
    END IF;
END $$;
