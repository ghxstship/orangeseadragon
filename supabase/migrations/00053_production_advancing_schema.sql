...
DROP TRIGGER IF EXISTS trg_advance_categories_updated_at ON advance_categories;
...
DROP TRIGGER IF EXISTS trg_production_advances_updated_at ON production_advances;
CREATE TRIGGER trg_production_advances_updated_at
    BEFORE UPDATE ON production_advances
    FOR EACH ROW EXECUTE FUNCTION update_advancing_updated_at();

DROP TRIGGER IF EXISTS trg_advance_items_updated_at ON advance_items;
CREATE TRIGGER trg_advance_items_updated_at
    BEFORE UPDATE ON advance_items
    FOR EACH ROW EXECUTE FUNCTION update_advancing_updated_at();

DROP TRIGGER IF EXISTS trg_advance_item_fulfillment_updated_at ON advance_item_fulfillment;
