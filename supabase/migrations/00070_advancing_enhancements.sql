...
CREATE TABLE IF NOT EXISTS crew_assignments (
    ...
    -- Rates
    rate_type VARCHAR(20) CHECK (rate_type IN ('hourly', 'day', 'flat', 'per_show')),
    ...
    status VARCHAR(20) DEFAULT 'pending' CHECK (status IN (
        ...
        'checked_in', 'checked_out', 'completed', 'no_show', 'cancelled'
    )),
...
-- ============================================================================
CREATE TRIGGER IF NOT EXISTS trg_asset_tags_updated_at
...
CREATE TRIGGER IF NOT EXISTS trg_crew_members_updated_at
    BEFORE UPDATE ON crew_members
    FOR EACH ROW EXECUTE FUNCTION update_advancing_updated_at();

CREATE TRIGGER IF NOT EXISTS trg_crew_availability_updated_at
    BEFORE UPDATE ON crew_availability
    FOR EACH ROW EXECUTE FUNCTION update_advancing_updated_at();

CREATE TRIGGER IF NOT EXISTS trg_crew_assignments_updated_at
