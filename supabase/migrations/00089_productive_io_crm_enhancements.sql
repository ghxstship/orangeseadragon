...
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE deals ADD COLUMN IF NOT EXISTS production_type VARCHAR(50);
    END IF;
END $$;
...
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE deals ADD COLUMN IF NOT EXISTS hold_status VARCHAR(30)
            CHECK (hold_status IN ('no_hold', 'first_hold', 'second_hold', 'confirmed', 'released', 'challenged'));
        ALTER TABLE deals ADD COLUMN IF NOT EXISTS hold_date TIMESTAMPTZ;
        ALTER TABLE deals ADD COLUMN IF NOT EXISTS hold_expires_at TIMESTAMPTZ;
        ALTER TABLE deals ADD COLUMN IF NOT EXISTS hold_notes TEXT;
    END IF;
END $$;
...
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE deals ADD COLUMN IF NOT EXISTS estimated_budget DECIMAL(14,2);
        ALTER TABLE deals ADD COLUMN IF NOT EXISTS estimated_margin_percent DECIMAL(5,2);
        ALTER TABLE deals ADD COLUMN IF NOT EXISTS estimated_costs DECIMAL(14,2);
    END IF;
END $$;
...
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE deals ADD COLUMN IF NOT EXISTS converted_by UUID REFERENCES users(id) ON DELETE SET NULL;
    END IF;
END $$;
...
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE deals ADD COLUMN IF NOT EXISTS referral_source VARCHAR(255);
        ALTER TABLE deals ADD COLUMN IF NOT EXISTS referral_contact_id UUID REFERENCES contacts(id) ON DELETE SET NULL;
    END IF;
END $$;
...
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE deals ADD COLUMN IF NOT EXISTS loss_reason VARCHAR(50)
            CHECK (loss_reason IN ('price', 'timing', 'competition', 'scope', 'relationship', 'budget_cut', 'no_decision', 'other'));
        ALTER TABLE deals ADD COLUMN IF NOT EXISTS loss_notes TEXT;
        ALTER TABLE deals ADD COLUMN IF NOT EXISTS competitor_name VARCHAR(255);
    END IF;
END $$;
...
BEGIN
    ...
    INSERT INTO projects (
        ...
        v_org_id,
        v_deal.name,
        lower(regexp_replace(v_deal.name, '[^a-zA-Z0-9]+', '-', 'g')),
