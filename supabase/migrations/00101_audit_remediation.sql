...
-- Create budget_scenarios table (referenced by history table below)
CREATE TABLE IF NOT EXISTS budget_scenarios (
    ...
    revenue_multiplier NUMERIC(5,2) DEFAULT 1.0,
    cost_multiplier NUMERIC(5,2) DEFAULT 1.0,
...
DROP POLICY IF EXISTS "Org members can read vendor payment schedule history" ON vendor_payment_schedules_history;
...
DROP POLICY IF EXISTS "Org members can read invoice automation rule history" ON invoice_automation_rules_history;
...
DROP POLICY IF EXISTS "Org members can read budget scenario history" ON budget_scenarios_history;
...
DROP TRIGGER IF EXISTS trg_vendor_payment_schedules_audit ON vendor_payment_schedules;
...
DROP TRIGGER IF EXISTS trg_invoice_automation_rules_audit ON invoice_automation_rules;
...
DROP TRIGGER IF EXISTS trg_budget_scenarios_audit ON budget_scenarios;
