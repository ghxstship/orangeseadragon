...
CREATE TABLE IF NOT EXISTS meal_penalties (
    ...
    user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    event_id UUID REFERENCES events(id) ON DELETE SET NULL,
    crew_checkin_id UUID REFERENCES crew_checkins(id) ON DELETE SET NULL,
...
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE time_entries ADD COLUMN labor_rule_set_id UUID REFERENCES labor_rule_sets(id) ON DELETE SET NULL;
        ALTER TABLE time_entries ADD COLUMN overtime_hours DECIMAL(6,2) DEFAULT 0;
        ALTER TABLE time_entries ADD COLUMN double_time_hours DECIMAL(6,2) DEFAULT 0;
        ALTER TABLE time_entries ADD COLUMN regular_hours DECIMAL(6,2) DEFAULT 0;
        ALTER TABLE time_entries ADD COLUMN total_cost DECIMAL(12,2) DEFAULT 0;
    END IF;
END $$;

-- Add labor_rule_set_id to crew_checkins
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_name = 'crew_checkins' AND column_name = 'labor_rule_set_id'
    ) THEN
        ALTER TABLE crew_checkins ADD COLUMN labor_rule_set_id UUID REFERENCES labor_rule_sets(id) ON DELETE SET NULL;
        ALTER TABLE crew_checkins ADD COLUMN meal_penalty_count INTEGER DEFAULT 0;
        ALTER TABLE crew_checkins ADD COLUMN meal_penalty_amount DECIMAL(10,2) DEFAULT 0;
    END IF;
END $$;
...
BEGIN
    IF EXISTS (
        ...
        IF NOT EXISTS (
            ...
            ALTER TABLE employee_profiles ADD COLUMN union_status VARCHAR(20) DEFAULT 'non_union'
                CHECK (union_status IN ('iatse', 'teamsters', 'ibew', 'ua', 'sag_aftra', 'non_union', 'other'));
            ALTER TABLE employee_profiles ADD COLUMN union_local VARCHAR(50);
            ALTER TABLE employee_profiles ADD COLUMN default_labor_rule_set_id UUID REFERENCES labor_rule_sets(id) ON DELETE SET NULL;
        END IF;
    END IF;
END $$;
...
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE time_entries ADD COLUMN timesheet_period_id UUID REFERENCES timesheet_periods(id) ON DELETE SET NULL;
    END IF;
END $$;
