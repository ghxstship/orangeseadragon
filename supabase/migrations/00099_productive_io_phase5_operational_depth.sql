...
GROUP BY cgr.organization_id, cgr.employee_id, u.full_name;
...
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE deals ADD COLUMN IF NOT EXISTS renewal_type VARCHAR(20)
        ...
        ALTER TABLE deals ADD COLUMN IF NOT EXISTS annual_value NUMERIC(14,2);
        ALTER TABLE deals ADD COLUMN IF NOT EXISTS total_contract_value NUMERIC(14,2);
        ALTER TABLE deals ADD COLUMN IF NOT EXISTS pitch_deck_document_id UUID REFERENCES documents(id) ON DELETE SET NULL;
    END IF;
END $$;
...
-- Requires projects.client_id column (not yet in schema)
-- Add client_id to projects first, then enable this view
-- ============================================================================

ALTER TABLE projects ADD COLUMN IF NOT EXISTS client_id UUID REFERENCES companies(id) ON DELETE SET NULL;
...
SELECT
    ...
    e.start_date,
    e.end_date,
    e.phase AS event_status,
    p.id AS project_id,
    p.name AS project_name,
    p.status AS project_status,
    b.total_amount AS total_budget,
    CAST(0 AS DECIMAL(14,2)) AS total_spent,
    inv.total_invoiced,
    inv.total_paid
FROM companies c
