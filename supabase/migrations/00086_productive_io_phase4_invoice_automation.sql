...
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE invoices ADD COLUMN source_settlement_id UUID REFERENCES settlements(id) ON DELETE SET NULL;
        ALTER TABLE invoices ADD COLUMN parent_invoice_id UUID REFERENCES invoices(id) ON DELETE SET NULL;
        ALTER TABLE invoices ADD COLUMN is_pass_through BOOLEAN DEFAULT FALSE;
        ALTER TABLE invoices ADD COLUMN pass_through_markup_pct NUMERIC(5,2) DEFAULT 0;
    END IF;
END $$;
