...
CREATE TABLE IF NOT EXISTS expense_approval_levels (
  ...
  approver_id UUID, -- user_id if approver_type is 'user'
  approver_role VARCHAR(100), -- role name if approver_type is 'role'
...
CREATE TABLE IF NOT EXISTS expense_approval_actions (
  ...
  level_order INTEGER NOT NULL,
  action VARCHAR(50) NOT NULL, -- 'approved', 'rejected', 'returned', 'escalated', 'skipped'
...
ADD COLUMN IF NOT EXISTS approval_status VARCHAR(50) DEFAULT 'draft', -- 'draft', 'pending', 'approved', 'rejected'
...
-- Create indexes
CREATE INDEX IF NOT EXISTS idx_expense_approval_policies_tenant ON expense_approval_policies(tenant_id);
...
BEGIN
  ...
  v_policy_id := get_expense_approval_policy(v_expense.tenant_id, v_expense.amount, v_expense.category);
...
INSERT INTO expense_approval_policies (tenant_id, name, description, min_amount, auto_approve_below, approval_levels, is_active)
SELECT 
  id,
