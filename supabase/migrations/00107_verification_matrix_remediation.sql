...
-- Extend existing fiscal_periods table (originally from 00024_gap_implementation_tables.sql)
...
ALTER TABLE fiscal_periods ADD COLUMN IF NOT EXISTS notes TEXT;
...
DROP POLICY IF EXISTS "fiscal_periods_org_isolation" ON fiscal_periods;
...
DROP POLICY IF EXISTS "revenue_recognitions_org_isolation" ON revenue_recognitions;
...
-- Extend existing venue_holds table (originally from 00089_productive_io_crm_enhancements.sql)
ALTER TABLE venue_holds ADD COLUMN IF NOT EXISTS converted_to_booking_id UUID;
ALTER TABLE venue_holds ADD COLUMN IF NOT EXISTS created_by UUID;
ALTER TABLE venue_holds ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMPTZ;

ALTER TABLE venue_holds ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "venue_holds_org_isolation" ON venue_holds;
...
DROP POLICY IF EXISTS "booking_splits_org_isolation" ON booking_splits;
...
DROP POLICY IF EXISTS "crew_offers_org_isolation" ON crew_offers;
...
DROP POLICY IF EXISTS "day_of_rosters_org_isolation" ON day_of_rosters;
...
DROP POLICY IF EXISTS "day_of_roster_entries_via_roster" ON day_of_roster_entries;
...
DROP POLICY IF EXISTS "digital_signatures_org_isolation" ON digital_signatures;
...
DROP POLICY IF EXISTS "invoice_reminder_steps_via_seq" ON invoice_reminder_steps;
...
DROP POLICY IF EXISTS "invoice_reminder_log_org" ON invoice_reminder_log;
...
DROP POLICY IF EXISTS "report_formula_fields_org" ON report_formula_fields;
...
-- Extend existing report_schedules table (originally from 00013_compliance_reports.sql)
ALTER TABLE report_schedules ADD COLUMN IF NOT EXISTS organization_id UUID;
ALTER TABLE report_schedules ADD COLUMN IF NOT EXISTS cron_expression TEXT;
ALTER TABLE report_schedules ADD COLUMN IF NOT EXISTS last_run_status TEXT CHECK (last_run_status IN ('success', 'failed', 'skipped'));
ALTER TABLE report_schedules ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMPTZ;

ALTER TABLE report_schedules ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "report_schedules_org" ON report_schedules;
...
DROP POLICY IF EXISTS "automation_run_log_org" ON automation_run_log;
...
-- Extend existing calendar_sync_connections table (originally from 00016_additional_entities.sql)
ALTER TABLE calendar_sync_connections ADD COLUMN IF NOT EXISTS organization_id UUID;
ALTER TABLE calendar_sync_connections ADD COLUMN IF NOT EXISTS external_calendar_name TEXT;
ALTER TABLE calendar_sync_connections ADD COLUMN IF NOT EXISTS sync_token TEXT;
ALTER TABLE calendar_sync_connections ADD COLUMN IF NOT EXISTS status TEXT DEFAULT 'active' CHECK (status IN ('active', 'paused', 'error', 'disconnected'));
ALTER TABLE calendar_sync_connections ADD COLUMN IF NOT EXISTS error_message TEXT;
ALTER TABLE calendar_sync_connections ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMPTZ;

ALTER TABLE calendar_sync_connections ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "calendar_sync_own" ON calendar_sync_connections;
...
DROP POLICY IF EXISTS "accounting_sync_org" ON accounting_sync_mappings;
