...
CREATE INDEX IF NOT EXISTS idx_quotas_active ON quotas(is_active) WHERE is_active = TRUE;
...
CREATE INDEX IF NOT EXISTS idx_forecast_snapshots_organization ON forecast_snapshots(organization_id);
CREATE INDEX IF NOT EXISTS idx_forecast_snapshots_date ON forecast_snapshots(snapshot_date DESC);
CREATE INDEX IF NOT EXISTS idx_forecast_snapshots_period ON forecast_snapshots(period_start, period_end);

-- ============================================================================
-- FORECAST SCENARIOS — extend existing table from 00012 with CRM columns
-- ============================================================================

ALTER TABLE forecast_scenarios ADD COLUMN IF NOT EXISTS organization_id UUID REFERENCES organizations(id) ON DELETE CASCADE;
ALTER TABLE forecast_scenarios ADD COLUMN IF NOT EXISTS probability_multiplier DECIMAL(5, 2) DEFAULT 1.0;
ALTER TABLE forecast_scenarios ADD COLUMN IF NOT EXISTS value_multiplier DECIMAL(5, 2) DEFAULT 1.0;
ALTER TABLE forecast_scenarios ADD COLUMN IF NOT EXISTS stage_adjustments JSONB DEFAULT '{}';
ALTER TABLE forecast_scenarios ADD COLUMN IF NOT EXISTS projected_revenue DECIMAL(14, 2);
ALTER TABLE forecast_scenarios ADD COLUMN IF NOT EXISTS projected_deals INTEGER;
ALTER TABLE forecast_scenarios ADD COLUMN IF NOT EXISTS period_start DATE;
ALTER TABLE forecast_scenarios ADD COLUMN IF NOT EXISTS period_end DATE;
ALTER TABLE forecast_scenarios ADD COLUMN IF NOT EXISTS is_active BOOLEAN DEFAULT TRUE;

CREATE INDEX IF NOT EXISTS idx_forecast_scenarios_organization ON forecast_scenarios(organization_id);
