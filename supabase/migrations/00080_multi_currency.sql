...
-- Extend existing currencies table (originally from 00023_missing_entity_tables.sql)
ALTER TABLE currencies ADD COLUMN IF NOT EXISTS organization_id UUID;
...
-- Create indexes
CREATE INDEX IF NOT EXISTS idx_currencies_tenant ON currencies(organization_id);
CREATE INDEX IF NOT EXISTS idx_currencies_code ON currencies(organization_id, code);
CREATE INDEX IF NOT EXISTS idx_exchange_rates_tenant ON exchange_rates(organization_id);
...
DROP POLICY IF EXISTS "Users can view currencies for their tenant" ON currencies;
...
DROP POLICY IF EXISTS "Users can manage currencies for their tenant" ON currencies;
...
DROP POLICY IF EXISTS "Users can manage exchange rates for their tenant" ON exchange_rates;
...
DECLARE
  v_rate DECIMAL(20,10);
BEGIN
  v_rate := get_exchange_rate(p_organization_id, p_from_currency, p_to_currency, p_date);
...
BEGIN
  ...
  IF NEW.currency != v_base_currency THEN
    NEW.exchange_rate := get_exchange_rate(NEW.organization_id, NEW.currency, v_base_currency, COALESCE(NEW.issue_date::DATE, CURRENT_DATE));
...
DROP TRIGGER IF EXISTS trg_invoice_base_amount ON invoices;
...
BEGIN
  ...
  -- Calculate base currency amount if different currency
  IF NEW.currency != v_base_currency THEN
    NEW.exchange_rate := get_exchange_rate(NEW.organization_id, NEW.currency, v_base_currency, COALESCE(NEW.date::DATE, CURRENT_DATE));
...
DROP TRIGGER IF EXISTS trg_expense_base_amount ON expenses;
...
-- Seed additional currencies (original table from 00023 already has USD, EUR, GBP, etc.)
