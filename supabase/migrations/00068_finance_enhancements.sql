...
CREATE INDEX idx_recurring_invoices_next_run ON recurring_invoices(next_run_date) WHERE is_active = TRUE;
...
CREATE INDEX idx_recurring_line_items_invoice ON recurring_invoice_line_items(recurring_invoice_id);
...
CREATE INDEX idx_reminder_templates_active ON reminder_templates(organization_id, is_active) WHERE is_active = TRUE;
...
CREATE INDEX idx_invoice_reminders_scheduled ON invoice_reminders(scheduled_date) WHERE status = 'pending';
...
CREATE INDEX idx_quotes_expiring ON quotes(organization_id, valid_until, status) 
...
CREATE INDEX idx_bank_connections_sync ON bank_connections(last_sync_at);
...
CREATE INDEX idx_imported_transactions_connection ON imported_transactions(bank_connection_id);
CREATE INDEX idx_imported_transactions_date ON imported_transactions(transaction_date);
CREATE INDEX idx_imported_transactions_status ON imported_transactions(match_status);
CREATE INDEX idx_imported_transactions_unmatched ON imported_transactions(bank_connection_id, match_status) 
...
CREATE INDEX idx_reconciliation_rules_org ON reconciliation_rules(organization_id);
CREATE INDEX idx_reconciliation_rules_active ON reconciliation_rules(organization_id, priority) WHERE is_active = TRUE;
...
CREATE INDEX idx_receipt_scans_org ON receipt_scans(organization_id);
...
CREATE INDEX idx_invoices_payment_link ON invoices(payment_link_token) WHERE payment_link_token IS NOT NULL;
...
CREATE INDEX idx_expenses_receipt_scan ON expenses(receipt_scan_id);
CREATE INDEX idx_expenses_imported_tx ON expenses(imported_transaction_id);
