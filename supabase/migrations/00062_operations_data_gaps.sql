...
-- Venue Zones
DROP POLICY IF EXISTS "venue_zones_org_access" ON venue_zones;
CREATE POLICY "venue_zones_org_access" ON venue_zones 
    FOR ALL USING (is_organization_member(organization_id));

-- Punch Lists
DROP POLICY IF EXISTS "punch_lists_org_access" ON punch_lists;
CREATE POLICY "punch_lists_org_access" ON punch_lists 
    FOR ALL USING (is_organization_member(organization_id));

DROP POLICY IF EXISTS "punch_list_items_org_access" ON punch_list_items;
...
-- Weather Reports
DROP POLICY IF EXISTS "weather_reports_org_access" ON weather_reports;
CREATE POLICY "weather_reports_org_access" ON weather_reports 
    FOR ALL USING (is_organization_member(organization_id));

-- Daily Reports
DROP POLICY IF EXISTS "daily_reports_org_access" ON daily_reports;
CREATE POLICY "daily_reports_org_access" ON daily_reports 
    FOR ALL USING (is_organization_member(organization_id));

DROP POLICY IF EXISTS "daily_report_sections_org_access" ON daily_report_sections;
...
DROP TRIGGER IF EXISTS trg_venue_zones_updated_at ON venue_zones;
CREATE TRIGGER trg_venue_zones_updated_at
    BEFORE UPDATE ON venue_zones
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS trg_punch_lists_updated_at ON punch_lists;
CREATE TRIGGER trg_punch_lists_updated_at
    BEFORE UPDATE ON punch_lists
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS trg_punch_list_items_updated_at ON punch_list_items;
...
DROP TRIGGER IF EXISTS trg_daily_report_sections_updated_at ON daily_report_sections;
