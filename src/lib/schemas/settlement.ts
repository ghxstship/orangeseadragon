...
export const settlementSchema = defineSchema({
  ...
  actions: { row: [{ key: 'view', label: 'View', handler: { type: 'navigate', path: (r: Record<string, unknown>) => `/modules/finance/settlements/${r.id}` } }], bulk: [], global: [{ key: 'create', label: 'New Settlement', variant: 'primary', handler: { type: 'function', fn: () => {} } }] },
