...
export const certificationSchema = defineSchema({
  ...
  layouts: { list: { subpages: [{ key: 'all', label: 'All', query: { where: {} }, count: true }, { key: 'valid', label: 'Valid', query: { where: { status: 'valid' } }, count: true }, { key: 'expiring', label: 'Expiring', query: { where: { status: 'valid', expiryDate: { lte: '{{now + 30d}}' } } }, count: true }, { key: 'expired', label: 'Expired', query: { where: { status: 'expired' } }, count: true }, { key: 'pending', label: 'Pending', query: { where: { status: 'pending' } } }], defaultView: 'table', availableViews: ['table'] }, detail: { tabs: [{ key: 'overview', label: 'Overview', content: { type: 'overview' } }], overview: { stats: [], blocks: [] } }, form: { sections: [{ key: 'basic', title: 'Certification Details', fields: ['name', 'personId', 'type', 'issuedDate', 'expiryDate', 'status', 'notes'] }] } },
  views: { table: { columns: [
    'name',
    { field: 'personId', format: { type: 'relation', entityType: 'person' } },
    { field: 'type', format: { type: 'badge', colorMap: { safety: '#ef4444', technical: '#3b82f6', medical: '#22c55e', license: '#8b5cf6' } } },
    { field: 'expiryDate', format: { type: 'date' } },
    { field: 'status', format: { type: 'badge', colorMap: { valid: '#22c55e', expired: '#ef4444', pending: '#eab308' } } },
  ] } },
