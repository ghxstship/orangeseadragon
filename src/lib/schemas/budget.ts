...
export const budgetSchema = defineSchema({
  ...
  actions: { row: [{ key: 'view', label: 'View', handler: { type: 'navigate', path: (r: Record<string, unknown>) => `/modules/finance/budgets/${r.id}` } }], bulk: [], global: [{ key: 'create', label: 'New Budget', variant: 'primary', handler: { type: 'function', fn: () => {} } }] },
