...
export const invoiceSchema = defineSchema({
  ...
  actions: { row: [{ key: 'view', label: 'View', handler: { type: 'navigate', path: (r: Record<string, unknown>) => `/modules/finance/invoices/${r.id}` } }], bulk: [], global: [{ key: 'create', label: 'New Invoice', variant: 'primary', handler: { type: 'function', fn: () => {} } }] },
