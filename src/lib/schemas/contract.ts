...
export const contractSchema = defineSchema({
  ...
  views: { table: { columns: ['title', 'companyId', 'value', 'status', 'startDate', 'endDate'] } },
  actions: { row: [{ key: 'view', label: 'View', handler: { type: 'navigate', path: (r: Record<string, unknown>) => `/modules/business/contracts/${r.id}` } }], bulk: [], global: [{ key: 'create', label: 'New Contract', variant: 'primary', handler: { type: 'function', fn: () => {} } }] },
