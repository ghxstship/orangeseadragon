...
export const incidentSchema = defineSchema({
  ...
  actions: { row: [{ key: 'view', label: 'View', handler: { type: 'navigate', path: (r: Record<string, unknown>) => `/modules/operations/incidents/${r.id}` } }], bulk: [], global: [{ key: 'create', label: 'Report Incident', variant: 'primary', handler: { type: 'function', fn: () => {} } }] },
