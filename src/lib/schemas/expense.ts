...
export const expenseSchema = defineSchema({
  identity: { name: 'expense', namePlural: 'Expenses', slug: 'modules/finance/expenses', icon: '💸', description: 'Expense tracking and reimbursements' },
  data: {
  ...
  actions: { row: [{ key: 'view', label: 'View', handler: { type: 'navigate', path: (r: Record<string, unknown>) => `/modules/finance/expenses/${r.id}` } }], bulk: [], global: [{ key: 'create', label: 'New Expense', variant: 'primary', handler: { type: 'function', fn: () => {} } }] },
