...
export async function POST(
  ...
  try {
    ...
    if (error) return error.code === 'PGRST116' ? notFound('Open shift') : supabaseError(error);
    return apiSuccess(data);
  } catch (err) {
    captureError(err, 'api.open-shifts.claim.error');
    return serverError('Failed to claim open shift');
  }
}
