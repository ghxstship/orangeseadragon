...
export async function POST(
  ...
  try {
    ...
    if (error) return error.code === 'PGRST116' ? notFound('Support ticket') : supabaseError(error);
    return apiSuccess(data);
  } catch (err) {
    captureError(err, 'api.support-tickets.close.error');
    return serverError('Failed to close support ticket');
  }
}
